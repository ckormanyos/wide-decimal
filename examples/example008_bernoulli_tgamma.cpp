...
namespace example008_bernoulli
{
  ...
  util::dynamic_array<wide_decimal_type> bernoulli_table(static_cast<std::uint_fast32_t>(static_cast<float>(std::numeric_limits<wide_decimal_type>::digits10) * 0.95F)); // NOLINT(cppcoreguidelines-avoid-magic-numbers,cppcoreguidelines-avoid-non-const-global-variables,cert-err58-cpp)
  ...
  auto tgamma(const FloatingPointType& x) -> FloatingPointType
  {
    ...
    if(xx > 8U) // NOLINT(cppcoreguidelines-avoid-magic-numbers)
    {
      ...
      auto fx_max = (std::max)(static_cast<floating_point_type>(8U), xx); // NOLINT(cppcoreguidelines-avoid-magic-numbers)
    ...
    for(auto n2 = static_cast<std::uint32_t>(4U); n2 < static_cast<std::uint32_t>(bernoulli_table.size()); n2 += 2U)
    {
      ...
      if((n2 > 6U) && (fabs(term) < tol)) // NOLINT(cppcoreguidelines-avoid-magic-numbers)
      {
        break;
      }

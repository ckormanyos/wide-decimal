...
bool math::wide_decimal::example009a_boost_math_standalone()
{
  ...
  const dec1001_t lp2 = local::legendre_p2(dec1001_t(1U) / 3, dec1001_t(1U) / 7, x);
  ...
  const dec1001_t closeness = fabs(1 - (lp2 / control));

  const bool result_is_ok = closeness < (std::numeric_limits<dec1001_t>::epsilon() * UINT32_C(10000000));
